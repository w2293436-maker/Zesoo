...
export default function AdminPage({ onBack }: AdminPageProps) {
  ...
  const handleExport = async (format: string) => {
    ...
    try {
      ...
      a.href = `/api/admin/export?password=${encodeURIComponent(pwd)}&format=${format}`;
      a.download = "";
  ...
  const filteredTasks = tasks.filter((t) =>
    !taskFilter || t.filename.toLowerCase().includes(taskFilter.toLowerCase())
  );
      ...
      <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => handleExport("json")} disabled={exporting === "json"}
            ...
            {exporting === "json" ? "导出中..." : (
              <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>JSON</>
            )}
          </button>
          <button onClick={() => handleExport("csv")} disabled={exporting === "csv"}
            className="flex items-center gap-1 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs text-gray-600 hover:bg-gray-50 disabled:opacity-50">
            {exporting === "csv" ? "导出中..." : (
              <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>CSV</>
            )}
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 max-w-6xl mx-auto w-full space-y-6">
        {!o ? (
          ...
          <>
            {/* ===== Section 1: 今日概览 ===== */}
            <section>
              <SectionTitle icon="📅" title="今日概览" />
              {t && (
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
                  <TodayCard label="访问" v={t.visits} color="blue" />
                  <TodayCard label="上传" v={t.uploads} color="amber" />
                  <TodayCard label="完成" v={t.completed} color="green" />
                  <TodayCard label="失败" v={t.fails} color="red" />
                  <TodayCard label="导出" v={t.exports} color="indigo" />
                  <TodayCard label="Token" v={fmtTokens(t.tokens)} color="purple" />
                </div>
              )}
            </section>
            ...
            {/* ===== Section 2: 累计核心指标 ===== */}
            <section>
              <SectionTitle icon="📊" title="累计核心指标" />
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <StatCard label="页面访问" v={o.total_visits.toLocaleString()} u="次" color="blue" />
                <StatCard label="书籍分析" v={o.completed.toLocaleString()} u="本" color="green" />
                <StatCard label="独立访客" v={o.unique_users.toLocaleString()} u="人" color="indigo" />
                <StatCard label="导出下载" v={o.total_exports.toLocaleString()} u="次" color="indigo" />
                <StatCard label="成功率" v={`${o.success_rate}%`} u="" color={o.success_rate >= 80 ? "green" : "amber"} />
                <StatCard label="导出率" v={`${o.export_rate}%`} u="" color={o.export_rate >= 50 ? "green" : "amber"} />
                <StatCard label="累计Token" v={fmtTokens(o.total_tokens)} u="" color="purple" />
                <StatCard label="估算费用" v={`¥${o.cost_est.toFixed(2)}`} u="" color="purple" />
                <StatCard label="处理字数" v={`${(o.total_chars / 10000).toFixed(1)}万`} u="" color="slate" />
                <StatCard label="累计耗时" v={`${o.total_time_hours.toFixed(1)}h`} u="" color="slate" />
                <StatCard label="OCR页数" v={o.ocr_pages.toLocaleString()} u="页" color="slate" />
                <StatCard label="平均耗时" v={`${o.avg_duration_seconds.toFixed(0)}s`} u="/本" color="slate" />
              </div>
            </section>
                    ...
                    <table className="w-full text-xs">
                      <thead className="bg-gray-50 text-gray-500">
                        <tr>
                          ...
                          <th className="text-right px-3 py-2 font-medium">访问</th>
                          <th className="text-right px-3 py-2 font-medium">上传</th>
                          <th className="text-right px-3 py-2 font-medium">完成</th>
                          <th className="text-right px-3 py-2 font-medium">Token</th>
                          <th className="text-right px-3 py-2 font-medium">导出</th>
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {stats.daily_trend.map((d, i) => (
                          <tr key={i} className="hover:bg-gray-50">
                            <td className="px-4 py-1.5 text-gray-600 font-medium">{d.date}</td>
                            <td className="px-3 py-1.5 text-right text-gray-500">{d.visits || "-"}</td>
                            <td className="px-3 py-1.5 text-right text-gray-500">{d.uploads || "-"}</td>
                            <td className="px-3 py-1.5 text-right text-green-600">{d.completed || "-"}</td>
                            <td className="px-3 py-1.5 text-right text-gray-500">{fmtTokens(d.tokens)}</td>
                            <td className="px-3 py-1.5 text-right text-gray-500">{d.exports || "-"}</td>
                            <td className="px-3 py-1.5 text-right text-red-400">{d.fails || "-"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex flex-col justify-center">
                    <h3 className="text-sm font-bold text-gray-700 mb-4">处理效率</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <EfficiencyCard label="平均每本耗时" v={`${o.avg_duration_seconds.toFixed(0)}s`} />
                      <EfficiencyCard label="平均每本Token" v={fmtTokens(o.completed > 0 ? Math.round(o.total_tokens / o.completed) : 0)} />
                      <EfficiencyCard label="平均每本字数" v={o.completed > 0 ? `${((o.total_chars / o.completed) / 10000).toFixed(1)}万` : "-"} />
                      <EfficiencyCard label="处理成功率" v={`${o.success_rate}%`} />
                    </div>
                  </div>
                </div>
              </section>
            ...
            {/* ===== Section 6: 任务记录 ===== */}
            <section>
              <SectionTitle icon="📋" title={`任务记录 (${filteredTasks.length})`} />
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-4 py-2.5 border-b border-gray-100">
                  <input
                    ...
                    className="w-full sm:w-64 px-3 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-400"
                  />
                </div>
                <TaskTable tasks={filteredTasks.slice(0, 30)} />
              </div>
            </section>
            ...
            {stats?.users && stats.users.length > 0 && (
              <section>
                <SectionTitle icon="👥" title={`独立访客 (${stats.users.length}人)`} />
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
...
function SectionTitle({ icon, title }: { icon: string; title: string }) {
  return (
    <h2 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-1.5">
      ...
      <span className="flex-1 border-t border-gray-100 ml-3" />
    </h2>
  );
}

function TodayCard({ label, v, color }: { label: string; v: number | string; color: string }) {
  const bgMap: Record<string, string> = {
    blue: "from-blue-50 to-blue-50 border-blue-100", amber: "from-amber-50 to-amber-50 border-amber-100",
    green: "from-green-50 to-green-50 border-green-100", red: "from-red-50 to-red-50 border-red-100",
    indigo: "from-indigo-50 to-indigo-50 border-indigo-100", purple: "from-purple-50 to-purple-50 border-purple-100",
  };
  const textMap: Record<string, string> = {
    blue: "text-blue-700", amber: "text-amber-700", green: "text-green-700",
    red: "text-red-700", indigo: "text-indigo-700", purple: "text-purple-700",
  };
  return (
    <div className={`rounded-xl bg-gradient-to-br ${bgMap[color] || bgMap.blue} border px-3 py-2.5 text-center`}>
      <div className="text-[10px] text-gray-500 mb-0.5">{label}</div>
      <div className={`text-lg font-bold ${textMap[color] || textMap.blue}`}>{v}</div>
    </div>
  );
}

function StatCard({ label, v, u, color }: { label: string; v: string; u: string; color: string }) {
  const bgMap: Record<string, string> = {
    ...
    indigo: "bg-indigo-50 border-indigo-100", slate: "bg-gray-50 border-gray-100",
  };
  const textMap: Record<string, string> = {
    blue: "text-blue-700", green: "text-green-700", amber: "text-amber-700",
    purple: "text-purple-700", indigo: "text-indigo-700", slate: "text-gray-700",
  };
  return (
    <div className={`rounded-xl border ${bgMap[color] || bgMap.slate} px-4 py-3`}>
      <div className="text-[10px] text-gray-500 mb-0.5">{label}</div>
      <div className={`text-xl font-bold ${textMap[color] || textMap.slate}`}>
        {v}<span className="text-xs font-normal ml-1 opacity-60">{u}</span>
      </div>
    </div>
  );
}

function EfficiencyCard({ label, v }: { label: string; v: string }) {
  return (
        ...
        {entries.map(([k, v]) => (
          <div key={k}>
            <div className="flex justify-between text-[11px] mb-1">
              <span className="text-gray-600 font-medium">{labels?.[k] || k.toUpperCase()}</span>
              <span className="text-gray-400">{v} ({total > 0 ? Math.round(v / total * 100) : 0}%)</span>
            </div>
...
function PhaseTimeBar({ phases }: { phases: Record<string, number> }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
      <h3 className="text-sm font-bold text-gray-700 mb-4">⏱ 各阶段平均耗时</h3>
      <div className="space-y-3">
        {Object.entries(phases).map(([k, v]) => (
          <div key={k}>
            <div className="flex justify-between text-xs mb-1">
              <span className="text-gray-600 font-medium">{labels[k] || k}</span>
              <span className="text-gray-500">{v}s ({total > 0 ? Math.round(v / total * 100) : 0}%)</span>
            </div>
            <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all"
                style={{ width: `${(v / total) * 100}%`, backgroundColor: colors[k] || "#6b7280" }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <table className="w-full text-xs sm:text-sm">
        <thead className="bg-gray-50 text-gray-500">
          <tr>
            ...
            <th className="text-right px-3 py-2 font-medium hidden sm:table-cell">字数</th>
            ...
            <th className="text-right px-3 py-2 font-medium hidden sm:table-cell">章节</th>
        ...
        <tbody className="divide-y divide-gray-50">
          {tasks.map((t) => (
            <tr key={t.task_id} className="hover:bg-gray-50">
              ...
              <td className="px-3 py-2 text-right text-gray-500 hidden sm:table-cell">{(t.chars / 1000).toFixed(0)}k</td>
              ...
              <td className="px-3 py-2 text-right text-gray-500 hidden sm:table-cell">{t.chapters || "-"}</td>
          ...
          {tasks.length === 0 && <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-400">无匹配任务</td></tr>}
        </tbody>
      </table>
    </div>
  );
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div className="flex flex-wrap gap-2">
            <Badge label="访问" v={user.visits} />
            <Badge label="上传" v={user.uploads} />
            <Badge label="完成" v={user.completed} c="green" />
            <Badge label="失败" v={user.failed} c="red" />
            <Badge label="导出" v={user.exports} />
            <Badge label="Token" v={fmtTokens(user.tokens_used)} />
          </div>
          <div className="text-xs text-gray-400 space-y-1 bg-gray-50 rounded-xl p-3">
            ...
            <p>首次: {fmtTime(user.first_seen)} · 最近: {fmtTime(user.last_seen)}</p>
          ...
          {tasks.length > 0 && (
            <div>
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {tasks.map((t) => (
                  <div key={t.task_id} className="flex items-center justify-between text-xs py-1.5 px-2 rounded-lg bg-gray-50 gap-2">
                    <span className="text-gray-600 truncate max-w-[120px]" title={t.filename}>{t.filename}</span>
                    <span className="text-gray-400 flex-shrink-0">{(t.chars / 1000).toFixed(0)}k字</span>
                    <span className="text-gray-400 flex-shrink-0">{t.duration_seconds ? `${t.duration_seconds.toFixed(0)}s` : ""}</span>
...
function Badge({ label, v, c }: { label: string; v: string | number; c?: string }) {
  const cc = c === "green" ? "bg-green-100 text-green-700" : c === "red" ? "bg-red-100 text-red-500" : "bg-gray-100 text-gray-700";
  return (
    <div className={`rounded-lg px-2.5 py-1 text-xs font-medium ${cc}`}>
      {label}: {v}
    </div>
  );
}
